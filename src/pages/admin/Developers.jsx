...
import { getDevelopersFromFirestore, saveDeveloper, deleteDeveloperFromFirestore } from "../../js/firebase";
import { toast } from "react-toastify";
import { motion } from "framer-motion";

export const adminConfig = {
  title: "Developer Administrator",
  description: "Manage developers and publishers.",
  icon: FaDev,
...
const AdminDevelopers = () => {
  ...
  const fetchDevelopers = async () => {
    try {
      setLoading(true);
      const data = await getDevelopersFromFirestore();
  ...
  const filteredDevelopers = useMemo(() => {
    ...
    return developers
      ...
      .map(dev => ({
        ...dev,
        parentName: dev.isStudio ? developers.find(d => d.id === dev.parentCompanyId)?.name : null,
        subsidiaryCount: dev.isStudio ? 0 : developers.filter(d => d.parentCompanyId === dev.id).length
      }));
  ...
  const alphabet = useMemo(() => {
    ...
    return letters.map(char => ({
      char,
      hasDevs: availableLetters.includes(char)
    }));
  }, [filteredDevelopers]);

  const scrollToLetter = (letter) => {
    const firstDev = filteredDevelopers.find(dev =>
      dev.name && dev.name.length > 0 && dev.name[0].toUpperCase() === letter
    );
    if (firstDev) {
      const element = document.getElementById(`dev-${firstDev.id}`);
      if (element) {
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        {editingDev ? (
          ...
          >
            ...
            {formData.isStudio && (
              <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-md">
                <h2 className="text-xl font-black mb-6 flex items-center gap-3 text-secondary-light">
                  <FaBuilding />
                  Parent Company
                </h2>
                <div className="relative">
                  <div className="relative group">
                    <FiSearch className="absolute left-6 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-primary transition-colors font-bold" />
                    <input
                      type="text"
                      value={parentSearch}
                      onChange={(e) => {
                  ...
                  {showParentSuggestions && parentSearch && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
                        ...
                        <div className="max-h-60 overflow-y-auto">
                          {parentSuggestions.map(parent => (
                            ...
                            >
                              ...
                              <span className="text-[10px] font-black uppercase text-white/20 group-hover:text-primary transition-colors">Select</span>
                            </button>
                          ))}
                        </div>
                      ) : (
                  ...
                  {formData.parentCompanyId && (
                    <div className="mt-4 flex items-center gap-4 bg-primary/10 border border-primary/20 rounded-2xl p-4">
                      <div className="size-10 bg-primary/20 rounded-xl flex items-center justify-center">
                        <FaCheckCircle className="text-primary" />
                      </div>
                      <div className="flex-1">
                        <p className="text-[10px] font-black uppercase text-primary tracking-widest">Selected Parent</p>
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-md">
              <h2 className="text-xl font-black mb-6 flex items-center gap-3 text-primary-light">
                <FaCity />
                Office Locations
              </h2>
              <div className="space-y-6">
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    className="bg-primary/20 hover:bg-primary/30 text-primary-light px-8 py-4 rounded-2xl font-black uppercase tracking-widest transition-all"
                  >
                    Add
                  </button>
                </div>
...
const DeveloperCard = memo(({ dev, handleEdit, handleDelete }) => {
  return (
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="p-3 bg-white/5 hover:bg-primary/20 rounded-xl transition-colors text-white/40 hover:text-white"
          >
            <FaEdit size={16} />
          </button>
          <button
      ...
      {(dev.isStudio || dev.subsidiaryCount > 0) && (
        <div className="mt-auto bg-black/20 rounded-2xl p-4 flex items-center justify-between border border-white/5">
          <span className="text-[10px] font-black uppercase tracking-widest text-white/40">
            {dev.isStudio ? "Parent Company" : "Subsidiaries"}
          </span>
          <span className="text-sm font-black text-primary-light">
            {dev.isStudio ? (dev.parentName || "N/A") : dev.subsidiaryCount}
          </span>
        </div>
      )}
    </motion.div>
